...
class Restore(utilities, Treeview_handle) :
    ...
    def __init__(self):
        ...
        # Advanced - Treeview for config tab
        ...
        for j in range(0,5) :
            ...
            cr[j].connect("edited", self.edit_list_search,j, "treeview4"); # cette procÃ©dure permet au cell-renderer de renvoyer le numÃ©ro de la colonne
        ...
        for j in range(0,7) :
            cr[j] = gtk.CellRendererText();
            cr[j].connect("edited", self.edit_list,j); # cette procÃ©dure permet au cell-renderer de renvoyer le numÃ©ro de la colonne
        ...
        for j in range(0,7) :
            cr[j] = gtk.CellRendererText();
            cr[j].connect("edited", self.edit_list_search,j,"treeview6"); # cette procÃ©dure permet au cell-renderer de renvoyer le numÃ©ro de la colonne
        ...
        for j in range(0,6) :
            cr[j] = gtk.CellRendererText();
            cr[j].connect("edited", self.edit_list_search,j,"treeview7"); # cette procÃ©dure permet au cell-renderer de renvoyer le numÃ©ro de la colonne
        ...
        for j in range(0,6) :
            cr[j] = gtk.CellRendererText();
            cr[j].connect("edited", self.edit_list_search,j,"gateway_details"); # cette procÃ©dure permet au cell-renderer de renvoyer le numÃ©ro de la colonne
...
##            alert(sprintf(_("Editing the field %s is not allowed."), champ));
    ...
    def edit_list_search(self, cr,row,texte,col, treeview) :
        ...
        if col != "" :
            ...
            if treeview == "treeview6" :
                ...
                self.config["xtabs"][name]["cols"][int(row)][champ] = texte   # +1 car cette liste commence à 1 et non à 0
            elif treeview == "treeview5" :
                name = get_sel_row_data(self.arw['result_lists'],0,0);
                v2(self.config["result"][name]["cols"], int(row))
                self.config["result"][name]["cols"][int(row)][champ] = texte   # +1 car cette liste commence à 1 et non à 0
                ...
                self.config["gateway_data"][name]["cols"][int(row)][champ] = texte   # +1 car cette liste commence à 1 et non à 0
    ...
    def show_selected(self, widget, *args) :
        # Advanced tab. When a line is selected, builds the proper line to
        # define the config item.
        # Example : it may return : config['central']['complete']['table']
        # And this will be used to update the dictionary "config"'
        ...
        while True :
            ...
            iter2 = model.iter_parent(iter1)
            if iter2 == None :
                break
            key2 = model.get_value(iter2,0)
            path.insert(0,key2)
            iter1 = iter2
        #place strings between quotes
        dico1 = self.config
        path1 = []
        for key in path :
            if key.isdigit() :
                if not key in dico1 :
                    if not int(key) in dico1 :
        ...
        #data += " = " + val1
        selected_path = data
        self.arw["treeview1_label"].set_text(data)
        command = "val2 = self.config" + data
        print command
        exec(command)
        if isinstance(val2,dict) :
            self.arw["treeview1_entry"].set_text("")
            self.arw["treeview1_entry"].set_editable(False)
        else :
            self.arw["treeview1_entry"].set_text(str(val2))
            self.arw["treeview1_entry"].set_editable(True)



    def update_field1(self, widget) :
        # used to edit the configuration full tree
        global selected_path
        val1 = self.arw["treeview1_entry"].get_text().strip()
        if not val1.isdigit() :
            val1 = val1.replace("'", "\\'")
            val1 = "'" + val1 + "'"
        command = "self.config" + selected_path + "=" + val1
        print command
        exec(command)
        self.save_settings("")

    def set_editable(self,*args) :
        # In order to add a field, it is necessary to set the entry field to "editable"
        self.arw["treeview1_entry"].set_editable(True)

    def add_field(self, widget) :
        global selected_path
        val1 = self.arw["add_entry1"].get_text().strip()
        if not val1.isdigit() :
            val1 = "'" + val1 + "'"
        command = "self.config" + selected_path + "[" + val1 + "]= ''"
        #print command
    ...
    def refresh_treeview1(self, widget) :
        self.model.clear()
        self.populate_tree(self.config, self.model)

    def delete_field(self, widget) :
        global selected_path

        command = "del self.config" + selected_path
        question = yes_no_dialog("Voulez-vous vraiment effacer ce champ : " + selected_path + "?");  # TODO
...
##                            node4 = config_store.append(node3,[data,"",""])
    ...
    def first_config(self, widget = None) :
        ...
        if config['result'] :               # TODO il y a ici un chargement en double : c'était déjà chargé par config_info['central_tables'][val['table']] = copy.deepcopy(val)
